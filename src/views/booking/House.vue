...
import {ellipsisVertical,house,trash,add} from 'ionicons/icons';
...
export default defineComponent({
    ...
    setup(){
        ...
        const state = reactive({
            tasksHouse: computed(() => {
                return store.getters.tasksByCategory('House');
            }),
            today: computed(() => {
                return store.getters.today(state.tasksHouse);
            }),
            late: computed(() => {
                return store.getters.late(state.tasksHouse);
            }),
            later: computed(() => {
                return store.getters.later(state.tasksHouse);
            }),
            done: computed(() => {
                return store.getters.done(state.tasksHouse);
            })
        })
        function getTasksHouse() {
        ...
        return{
            isOpenNewTask,store,state,getTasksHouse,doneTask,notDoneTask,deleteTask,
            ellipsisVertical,house,trash,add
        }
    }
