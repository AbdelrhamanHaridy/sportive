    ...
    <ion-page>
        ...
        <ion-content class="overflow-auto">
            <div class="flex flex-col justify-center items-center mt-2">
                ...
                <div class="text-center">
                    <ion-card-title class="text-2xl">Swimming</ion-card-title>
                    <ion-card-subtitle>{{state.tasksSwim.length}} Appointment</ion-card-subtitle>
                </div>
            </div>
...
export default defineComponent({
    ...
    setup(){
      ...
      const state = reactive({
        tasksSwim: computed(() => {
            return store.getters.tasksByCategory('Swim')
        }),
        today: computed(() => {
            return store.getters.today(state.tasksSwim);
        }),
        late: computed(() => {
            return store.getters.late(state.tasksSwim);
        }),
        later: computed(() => {
            return store.getters.later(state.tasksSwim);
        }),
        done: computed(() => {
            return store.getters.done(state.tasksSwim);
        })
      }); 
      function getTasksSwim() {
    ...
    onMounted(() => {
      ...
      if (store.state.tasks.length == 0) {
        ...
        return{
          state,getTasksSwim,doneTask,notDoneTask,store,isOpenNewTask,deleteTask,
