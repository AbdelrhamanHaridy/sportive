        ...
        <ion-content class="overflow-auto">
            <div class="flex flex-col justify-center items-center mt-2">
                ...
                <div class="text-center">
                    ...
                    <ion-card-subtitle>{{state.tasksRunning.length}} Appointment</ion-card-subtitle>
                </div>
            </div>
...
import {ellipsisVertical,Running,trash,add} from 'ionicons/icons';
...
export default defineComponent({
    ...
    setup(){
        ...
        const state = reactive({
            tasksRunning: computed(() => {
                return store.getters.tasksByCategory('Running');
            }),
            today: computed(() => {
                return store.getters.today(state.tasksRunning);
            }),
            late: computed(() => {
                return store.getters.late(state.tasksRunning);
            }),
            later: computed(() => {
                return store.getters.later(state.tasksRunning);
            }),
            done: computed(() => {
                return store.getters.done(state.tasksRunning);
            })
        })
        ...
        return{
            isOpenNewTask,store,state,getTasksRunning,doneTask,notDoneTask,deleteTask,
            ellipsisVertical,Running,trash,add
        }
    }
