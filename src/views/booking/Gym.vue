        ...
        <ion-content class="overflow-auto">
            <div class="flex flex-col justify-center items-center mt-2">
                ...
                <div class="text-center">
                    ...
                    <ion-card-subtitle>{{state.tasksGym.length}} Appointment</ion-card-subtitle>
                </div>
            </div>
...
export default defineComponent({
    ...
    setup(){
        ...
        const state = reactive({
            tasksGym: computed(() => {
                return store.getters.tasksByCategory('Gym');
            }),
            today: computed(() => {
                return store.getters.today(state.tasksGym);
            }),
            late: computed(() => {
                return store.getters.late(state.tasksGym);
            }),
            later: computed(() => {
                return store.getters.later(state.tasksGym);
            }),
            done: computed(() => {
                return store.getters.done(state.tasksGym);
            })
        })
        function getTasksGym() {
        ...
        return{
            isOpenNewTask,store,state,getTasksGym,doneTask,notDoneTask,deleteTask,
