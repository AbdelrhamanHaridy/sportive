        ...
        <ion-content class="overflow-auto">
            <div class="flex flex-col justify-center items-center mt-2">
                ...
                <div class="text-center">
                    ...
                    <ion-card-subtitle>{{state.tasksBang.length}} Appointment</ion-card-subtitle>
                </div>
            </div>
...
export default defineComponent({
    ...
    setup(){
      ...
      const state = reactive({
        tasksBang: computed(() => {
          return store.getters.tasksByCategory('Bang');
        }),
        today: computed(() => {
            return store.getters.today(state.tasksBang);
        }),
        late: computed(() => {
            return store.getters.late(state.tasksBang);
        }),
        later: computed(() => {
            return store.getters.later(state.tasksBang);
        }),
        done: computed(() => {
            return store.getters.done(state.tasksBang);
        })
      }); 
      function getTasksBang() {
    ...
    onMounted(() => {
      ...
      return{
        state,getTasksBang,store,doneTask,notDoneTask,isOpenNewTask,deleteTask,
